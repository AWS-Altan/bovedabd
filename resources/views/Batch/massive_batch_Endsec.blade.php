			...
			<div class="panel-wrapper collapse in">
				<div class="panel-body">
					<div id="example-basic">
						<h3><span class="head-font capitalize-font">Termino de sesion masivo de Usuarios</span></h3>
						<section>
...
<script>
    ...
    ******************************/
    function loadfile($file_data){
	    ...
	    $.ajax({
            url: "{{ route('batch.masive_endsec.load') }}",
            type: 'POST',
    ...
    ******************************/
    function executeCharge($file_data){
        ...
        $.ajax({
            url: "{{ route('batch.masive_endsec.exec') }}",
            type: 'POST',
        ...
        if(!bJL_successLoad)
        {
            ...
            if ($('#inputFileData').val() )
            {
                ...
                if ( extension.toUpperCase() != 'ENDSESSION')
                {
                    ...
                    $('#inputErrors').show();
                    $('#result').empty();
                    $('#result').append('<label class="help-block mb-30 text-left" style="color: red"><strong>Formato incorrecto, sólo se permite archivos de fin de session</strong>');
                    return;
                }//if
